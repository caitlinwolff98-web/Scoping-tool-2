...
' VERSION: 2.0 - Updated to work with dictionary-based analysis
'==============================================================================
...
Private Sub BuildFullTableStructure(ByVal ws As Worksheet, ByVal fslis As Collection, ByVal packs As Collection, ByVal segmentTabs As Collection)
    ...
    For Each segmentWs In segmentTabs
        ...
        ' Analyze Row 6 to get valid columns (returns Dictionary)
        Dim lastCol As Long
        ...
        For Each colKey In colAnalysis.Keys
            analysisArray = colAnalysis(colKey)
            ' analysisArray format: (0)=IsValid, (1)=MarkerText, (2)=PackName, (3)=PackCode
            If analysisArray(0) = True And analysisArray(3) <> "" Then
                If Not packColumnMap.Exists(analysisArray(3)) Then
                    packColumnMap.Add analysisArray(3), CLng(colKey)
                End If
            End If
        Next colKey
