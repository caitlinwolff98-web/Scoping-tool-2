...
' VERSION: 2.0 - Simplified without class modules
'==============================================================================

'==================== ROW 6 ANALYSIS ====================

' Analyze Row 6 to identify which columns should be included/excluded
' Returns: Dictionary with column index as key, and array of analysis results as value
' Array format: (0)=IsValid, (1)=MarkerText, (2)=PackName, (3)=PackCode
Public Function AnalyzeRow6(ByVal ws As Worksheet, ByVal startCol As Long, ByVal endCol As Long) As Object
    On Error GoTo ErrorHandler

    LogMessage "Analyzing Row 6 for worksheet: " & ws.Name

    Dim results As Object
    Set results = CreateDictionary()
    ...
    For colIdx = startCol To endCol
        ...
        markerText = SafeTrim(ws.Cells(ROW_MARKER, colIdx).Value)
        isValid = False
        packName = ""
        packCode = ""
        ...
        If markerText = "" Then
            ...
            isValid = True
        ElseIf ContainsExclusionMarker(markerText) Then
            ' Contains exclusion marker = skip this column
            isValid = False
            LogMessage "  Column " & colIdx & " EXCLUDED (Row 6: '" & markerText & "')"
        Else
        ...
        If isValid Then
            ...
            If row8Value <> "" Then
                ...
                Call SeparatePackNameAndCode(row8Value, packName, packCode)
                LogMessage "  Column " & colIdx & " INCLUDED - Pack: " & packName & " (Code: " & packCode & ")"
            Else
                ' No pack data in Row 8
                isValid = False ' Skip columns without pack data
            End If
        End If

        ' Store results as array in dictionary
        results.Add colIdx, Array(isValid, markerText, packName, packCode)
    Next colIdx
...
ErrorHandler:
    ...
    Set AnalyzeRow6 = CreateDictionary()
End Function

' Count valid columns from analysis results dictionary
Private Function CountValidColumns(ByVal results As Object) As Long
    Dim count As Long
    Dim key As Variant
    ...
    For Each key In results.Keys
        analysisArray = results(key)
        If analysisArray(0) = True Then count = count + 1 ' Check IsValid
    Next key
...
Public Function CollectAllPacks(ByVal segmentWorksheets As Collection) As Collection
    ...
    For Each wsVar In segmentWorksheets
        ...
        For Each colIdx In colAnalysis.Keys
            analysisArray = colAnalysis(colIdx)

            ' Check if valid and has pack name
            If analysisArray(0) = True And analysisArray(2) <> "" Then
                packName = analysisArray(2)
                packCode = analysisArray(3)
                packKey = packCode & "|" & packName

                If Not packDict.Exists(packKey) Then
                    packDict.Add packKey, Array(packName, packCode)

                    ' Also add to global pack list
                    If Not g_PackList.Exists(packKey) Then
                        g_PackList.Add packKey, Array(packName, packCode)
                    End If
                End If
            End If
        Next colIdx
...
' Get analysis result as dictionary for easier access
' analysisArray format: (0)=IsValid, (1)=MarkerText, (2)=PackName, (3)=PackCode
Public Function GetAnalysisIsValid(ByVal analysisArray As Variant) As Boolean
    GetAnalysisIsValid = analysisArray(0)
End Function

Public Function GetAnalysisPackName(ByVal analysisArray As Variant) As String
    GetAnalysisPackName = analysisArray(2)
End Function

Public Function GetAnalysisPackCode(ByVal analysisArray As Variant) As String
    GetAnalysisPackCode = analysisArray(3)
End Function

Public Function GetAnalysisColumnIndex(ByVal colIdx As Variant) As Long
    GetAnalysisColumnIndex = CLng(colIdx)
End Function
